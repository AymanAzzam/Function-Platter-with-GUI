...
class Form(QDialog):
    ...
    def Update(self):
        ...
        if err:
            QTimer.singleShot(2500, self.__UpdateMessage)
            return
    ...
    def TestFunction(self, Qtbot):
        self.function.setText("X^2+sin(x)")
        Qtbot.mouseClick(self.button, Qt.LeftButton)
        assert str(self.result_label.text()).count("Error") > 0
